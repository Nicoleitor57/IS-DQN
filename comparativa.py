...
#                         choices=['TwoTigersEnv', 'KeyDoorMazeEnv', 'PODoorEnv', 'DelayedObsEnv', 'DelayedObsEnv-k5', 'DelayedStochasticObsEnv'],
...
def load_log_data(base_path, env_name, num_runs):
    """
    Carga los datos de 'monitor.csv' para un algoritmo y entorno específico.
    Soporta carpetas estándar (base_path/env_name/run_i) y carpetas arbitrarias (base_path/run_i).
    """

    # --- NUEVO: permitir carpetas arbitrarias sin subcarpeta del entorno ---
    if os.path.exists(os.path.join(base_path, env_name)):
        target_dir = os.path.join(base_path, env_name)
    else:
...
def plot_comparison(env_name_base, num_runs, smooth_window, tipo_variant=None,
                    max_episodes_to_plot=None, extra_exps=None):
    """
    Genera los gráficos comparativos, incluyendo experimentos arbitrarios si se especifican.
    """
    ...
    # --- NUEVO: agregar carpetas adicionales arbitrarias ---
    if extra_exps:
        for exp_path in extra_exps:
            algo_name = f"Extra: {os.path.basename(exp_path.rstrip('/'))}"
            algos[algo_name] = exp_path
    ...
    # Cargar datos de todos los algoritmos
    for algo_name, base_path in algos.items():
        rewards, lengths, min_ep_len_algo, valid_runs = load_log_data(base_path, env_name_search, num_runs)
    ...
    if global_min_ep_len == float('inf'):
        print("\nERROR: No se encontró información válida para generar la gráfica.")
        return
    ...
    if final_ep_len == 0:
        print("\nAdvertencia: Límite de episodios es cero o no hay datos disponibles.")
        return
    ...
    for algo_name, data in algo_data.items():
        ...
        rewards_final = data['rewards'][:, :final_ep_len]
        lengths_final = data['lengths'][:, :final_ep_len]
        ...
        mean_r = np.mean(rewards_smooth, axis=0)
        std_r = np.std(rewards_final, axis=0)

        mean_l = np.mean(lengths_smooth, axis=0)
        std_l = np.std(lengths_final, axis=0)
    ...
    if not plotted_any:
        print("\nERROR: Ningún algoritmo produjo datos para graficar.")
        return
...
def main():
    ...
    parser.add_argument('--env', type=str, required=True,
                        choices=['TwoTigersEnv', 'KeyDoorMazeEnv', 'PODoorEnv',
                                 'DelayedObsEnv', 'DelayedObsEnv-k5', 'DelayedStochasticObsEnv'],
                        help='Nombre base del entorno.')

    parser.add_argument('--runs', type=int, default=30,
                        help='Número máximo de corridas.')

    parser.add_argument('--smooth', type=int, default=20,
                        help='Ventana de suavizado.')

    parser.add_argument('--tipo', type=str, default=None,
                        choices=['corto', 'largo', '100', '1'],
                        help='Sufijo opcional para el entorno.')

    parser.add_argument('--max-episodes', type=int, default=None,
                        help='Número máximo de episodios a graficar.')

    # --- NUEVO: Experimentos arbitrarios ---
    parser.add_argument('--extra-exps', nargs='*', default=[],
                        help='Carpetas extra con runs y monitor.csv')
    ...
    plot_comparison(args.env, args.runs, args.smooth,
                    args.tipo, args.max_episodes, args.extra_exps)
